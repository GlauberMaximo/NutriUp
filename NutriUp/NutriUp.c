...
void cadastro_de_pacientes() {
    if(total_paciente >= MAX_PACIENTES){
        printf("Limite de pacientes atigindo \n");
        return;
    }
    Paciente P;
    P.id = total_paciente  + 1;
    P.ativo = 1;
    ...
    scanf(" %[^\n]" P.conta_paciente.usuario);
    printf("Senha do Paciente: \n"); //senha
    scanf(" %[^\n]" P.conta_paciente.senha);

    P.conta_paciente.tipo_acesso = 2; // 1=Paciente/2=Funcionario

    pacientes[total_paciente++] = P;
}
    limpar_tela();
